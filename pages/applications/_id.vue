  ...
  <div class="create-hotel max-w-[1536px] mx-auto py-[60px]">
    <a-form-model ref="ruleForm" :model="form" :rules="rules">
      ...
      <div class="body mt-10 flex flex-col gap-10">
        ...
        <!--        <div-->
        <!--          v-if="files?.status !== 'accepted' &&-->
        <!--                files?.status !== 'rejected' &&-->
        <!--                files?.pipe_status === null &&-->
        <!--                files?.status !== 'new' &&-->
      ...
      >
        <div>
          ...
          <div class="buttons grid grid-cols-2 gap-[30px] mt-4">
            <a-button
              :loading="loading"
              class="v-btn danger-btn py-[13px] rounded-[8px] text-white bg-red-dark2 font-[verdana-400] text-base uppercase flex justify-center"
              @click="handleOk"
            >
              Bekor qilish
            </a-button>
            <a-button
              :loading="loading"
              class="v-btn blue-btn py-[13px] rounded-[8px] text-white bg-blue-bold font-[verdana-400] text-base uppercase flex justify-center"
  ...
  methods: {
    ...
    async __GET_APPLICATIONS(id) {
      try {
        ...
        if (newApplication) {
          hotel = newApplication?.changes?.hotel;
          this.files = {...this.files, ...data?.data?.changes}
        }
        this.form.address = hotel?.address
          ? hotel?.address
          : {
            uz: "",
            en: "",
        ...
        this.$refs.appSteps.aplicationFormGettings()
      } catch (e) {
      }
    },
