            ...
            <div class="flex flex-col gap-10">
              <div class="grid grid-cols-1 w-full">
                <a-form-model-item
                  prop="name.uz"
                  class="form-item w-full mb-0"
                  label="Xizmat nomi (O‘Z)"
                >
                  <a-input v-model="form.name.uz" placeholder="Matn..." />
                </a-form-model-item>
              </div>
              <div class="grid grid-cols-1 w-full">
                <a-form-model-item class="form-item w-full mb-0" label="Xizmat nomi (RU)">
...
export default {
  ...
  methods: {
    ...
    async __GET_REASONS() {
      try {
        this.loading = true;
        const data = await this.$store.dispatch("fetchAddServices/getAddServices", {
          page: 1,
          page_size: 16,
          ...this.$route.query,
        });
    ...
    async __DELETE_CHAPTERS(id) {
      try {
        const data = await this.$store.dispatch("fetchAddServices/deleteAddServices", id);
        this.__GET_REASONS();
    ...
    async __GET_REASON(id) {
      try {
        const data = await this.$store.dispatch(
          "fetchAddServices/getAddServicesById",
          id
        );
    ...
    async __POST_REASONS(form) {
      try {
        const data = await this.$store.dispatch("fetchAddServices/postAddServices", {
          data: form,
        });
        this.__GET_REASONS();
        this.editId = null;
    ...
    async __EDIT_REGION(form) {
      try {
        const data = await this.$store.dispatch("fetchAddServices/editAddServices", {
          id: this.editId,
          data: form,
        });
        this.__GET_REASONS();
